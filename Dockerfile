# ─────────────────────────────────────────────────────────────────────────────
# Belgian Data Protection MCP — multi-stage Dockerfile
# ─────────────────────────────────────────────────────────────────────────────
# Build:  docker build -t belgian-data-protection-mcp .
# Run:    docker run --rm -p 3000:3000 belgian-data-protection-mcp
#
# The image bakes /app/data/gba.db at build time. Override GBA_DB_PATH for
# a custom location at runtime.
# ─────────────────────────────────────────────────────────────────────────────
...
ENV GBA_DB_PATH=/app/data/gba.db
...
# Bake the pre-built database into the image so /app/data/gba.db resolves
...
# We then COPY it into the image at /app/data/gba.db (GBA_DB_PATH). The
# explicit `data/<name>.db` reference is required for the workflow's grep
# `COPY\s+\K(data/\S+\.db)` to match.
COPY data/database.db data/gba.db
